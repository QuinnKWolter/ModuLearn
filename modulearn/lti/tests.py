...
class LTIOutcomeProgressIntegrationTestCase(TestCase):
    def setUp(self):
        self.client = Client()
        ...
        self.course = Course.objects.create(id='lti-course', title='LTI Course')
        self.instance = CourseInstance.objects.create(course=self.course, group_name='LTI Session')
        self.unit = Unit.objects.create(course=self.course, title='Unit A')
        self.module = Module.objects.create(unit=self.unit, title='Tool Module')
        self.enrollment = Enrollment.objects.create(student=self.user, course_instance=self.instance)

        self.source_id = f'{self.user.id}_{self.instance.id}_activity1'
        LTILaunchCache.get_or_create_cache(
            source_id=self.source_id,
            tool='codecheck',
            usr=str(self.user.id),
            grp=str(self.instance.id),
            sub='activity1',
    ...
    @patch.dict(os.environ, {
        'CODECHECK_KEY': 'k',
        'CODECHECK_SECRET': 's',
        'CODECHECK_LAUNCH': 'https://cc.io',
    })
    def test_outcome_updates_module_progress_and_event_ledger(self, mock_get):
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_get.return_value = mock_response
        ...
        <imsx_POXEnvelopeRequest xmlns="http://www.imsglobal.org/services/ltiv1p1/xsd/imsoms_v1p0">
          <imsx_POXBody>
            <replaceResultRequest>
              <resultRecord>
                <sourcedGUID>
                  <sourcedId>{self.source_id}</sourcedId>
                </sourcedGUID>
                <result>
                  <resultScore>
        ...
        response = self.client.post(reverse('lti_outcome'), data=xml, content_type='application/xml')
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'success', response.content)

        progress = ModuleProgress.objects.get(enrollment=self.enrollment, module=self.module)
        self.assertTrue(progress.is_complete)
        self.assertEqual(progress.score, 100.0)
        self.assertEqual(progress.progress, 1.0)
        self.assertIsNotNone(progress.completed_at)

        event_types = set(ModuleProgressEvent.objects.filter(module_progress=progress).values_list('event_type', flat=True))
        self.assertIn('outcome', event_types)
