...
def get_user_groups_with_masterygrids_nodes(user) -> List[Dict[str, Any]]:
    ...
    # If node enrichment fails, preserve the already-correct group_login/course_ids payload
    # so dashboards can still launch analytics and discover resources.
    ...
    try:
        ...
        logger.warning(
            f"Failed to enrich MasteryGrids node IDs for user {user.username}; keeping group/course mappings only: {str(e)}",
            exc_info=True,
        )
        node_id_mappings = {}
